...
def generate_feat_map(gplot, subduction_data, projection, time, feature, output_dir):
    ...
    sc = ax.scatter(subduction_data_t['lon'], subduction_data_t['lat'], 50, marker='.',
                    c=subduction_data_t[feature], cmap=ccm.hawaii_r, transform=ccrs.PlateCarree(), zorder=5)
    ...
    custom_handles = [
        ...
        Line2D([0], [0], color='red', lw=2, label='Mid-Ocean Ridge')
    ]
    ax.legend(handles=custom_handles, loc='lower left')
    ax.set_title(f"CAO24 Subduction Zones {time} Ma", fontsize=25, y=1.04)

    filename = os.path.join(output_dir, f"CAO24_map_{time:.0f}Ma.png")
