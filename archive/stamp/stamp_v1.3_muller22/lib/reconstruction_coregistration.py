...
def _multiple_timesteps_buffer_parallel(
    times: Sequence[float],
    buffer_distance: float,
    return_output: bool,
    rotation_model: Optional[_RotationModelInput] = None,
...
def _generate_points_timestep(
    ...
    if (
        ...
        if time == 0.0:
            ...
            present_day_coords = _reconstruct_by_topologies_present(
                data=pd.DataFrame(
                    {
                        "lon": points[:, 0],
                        "lat": points[:, 1],
...
def _reconstruct_by_topologies_present(
    data,
    plate_reconstruction=None,
    rotation_model=None,
    topology_features=None,
    ...
    reconstructed_points = reconstruct_points(
        rotation_model,
        topology_features,
        reconstruction_begin_time=reconstruction_time,
        reconstruction_end_time=0.0,
        reconstruction_time_interval=1.0,
        points=points,
        detect_collisions=None,
    )

    new_lats, new_lons = zip(*[p.to_lat_lon() for p in reconstructed_points])
    data["lon_0"] = new_lons
    data["lat_0"] = new_lats
