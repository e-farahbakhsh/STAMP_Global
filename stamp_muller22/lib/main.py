...
def downsample_unlabelled(
    ...
    if len(unlabelled_data) < multiplier * len(deposit_data):
        print("No downsampling required!")
        return unlabelled_data
    else:
        n_target = multiplier * len(deposit_data)
    
    if not isinstance(random_state, np.random.Generator):
        random_state = np.random.default_rng(random_state)
    
    # Bin age (temporal distribution)
    unlabelled_data = unlabelled_data.copy()
    unlabelled_data['age_bin'] = pd.qcut(unlabelled_data['age (Ma)'], q=n_bins_age, duplicates='drop')
    
    # Bin spatial data (spatial distribution)
    unlabelled_data['lon_bin'] = pd.cut(unlabelled_data['lon'], bins=n_bins_spatial)
    unlabelled_data['lat_bin'] = pd.cut(unlabelled_data['lat'], bins=n_bins_spatial)
    
    # Group by combined bins
    grouped = unlabelled_data.groupby(['age_bin', 'lon_bin', 'lat_bin'])
    
    # Calculate how many samples to draw from each bin
    total_bins = len(grouped)
    samples_per_bin = max(1, n_target // total_bins)
    
    sampled_indices = []
    for _, group in grouped:
        n_samples = min(samples_per_bin, len(group))
        sampled_indices.extend(random_state.choice(group.index, size=n_samples, replace=False))

    sampled_unlabelled = unlabelled_data.loc[sampled_indices]
    sampled_unlabelled = sampled_unlabelled.drop(columns=['age_bin', 'lon_bin', 'lat_bin'])
    
    return sampled_unlabelled
