...
from lib.main import reconstruct_by_topologies
...
def plot(
    ...
    raster_probs = Raster(probs_grid)
    raster_probs.data *= 100.0
    ...
    sm_probs, sm_bg = _prepare_axes(
        ax=ax,
        gplot=gplot,
        raster_probs=raster_probs,
        raster_bg=raster_bg,
        time=time,
        positives=positives,
    ...
    cax_probs = fig.add_axes([0.1, 0.12, 0.35, 0.02])
    cax_bg = fig.add_axes([0.55, 0.12, 0.35, 0.02])
        
    sm_probs.set_clim(probs_kwargs["vmin"], probs_kwargs["vmax"])
    sm_bg.set_clim(background_kwargs["vmin"], background_kwargs["vmax"])
    
    cbar_probs = fig.colorbar(sm_probs, cax=cax_probs, orientation="horizontal")
    cbar_probs.set_label("Mineralisation Probability (%)", fontsize=FONT_SIZE, labelpad=10)
    cbar_probs.set_ticks([0, 20, 40, 60, 80, 100])
    cbar_probs.ax.tick_params(labelsize=TICK_SIZE)

    cbar_bg = fig.colorbar(sm_bg, cax=cax_bg, orientation="horizontal", extend='max')
    ...
    Line2D([0], [0], color=topologies_kwargs["color"], lw=2, alpha=0.5, label="Topological Boundaries"),
...
def _prepare_axes(
    ...
    **kwargs,
    ...
    sm_probs = ScalarMappable(
        norm=Normalize(
            probs_kwargs["vmin"],
            probs_kwargs["vmax"],
        ),
        cmap=probs_kwargs["cmap"],
    )
    
    sm_bg = ScalarMappable(
        norm=Normalize(
            background_kwargs["vmin"],
            background_kwargs["vmax"],
        ),
        cmap=background_kwargs["cmap"],
    )

    return sm_probs, sm_bg
